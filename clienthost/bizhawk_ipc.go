...
func ReserveLuaPort() (int, error) {
	...
	for {
		...
		if err != nil {
			port++
			if port > 65535 {
				return 0, fmt.Errorf("no free lua port found")
			}
			continue
		}
		_ = ln.Close()
...
// WriteLuaPortFile writes the port for server.lua to read at BizHawk launch.
func WriteLuaPortFile(dataDir string, port int) error {
	if err := os.MkdirAll(dataDir, 0o755); err != nil {
		return err
	}
	path := filepath.Join(dataDir, "lua_server_port.txt")
...
func (b *BizhawkIPC) SendAutoSaveDisable(ctx context.Context) error {
