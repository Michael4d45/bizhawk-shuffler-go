...
type DiscoveryListener struct {
	config        *protocol.DiscoveryConfig
	discovered    map[string]*protocol.ServerInfo
	mu            sync.RWMutex
	conn          *net.UDPConn
	localConn     *net.UDPConn // Store localhost connection for cleanup
	running       bool
	cancel        context.CancelFunc
	onServerFound func(*protocol.ServerInfo) // Callback for when a server is discovered
}

// NewDiscoveryListener creates a new discovery listener
func NewDiscoveryListener(config *protocol.DiscoveryConfig) *DiscoveryListener {
	return &DiscoveryListener{
		config:     config,
		discovered: make(map[string]*protocol.ServerInfo),
	}
}
...
func (dl *DiscoveryListener) GetDiscoveredServers() []*protocol.ServerInfo {
	dl.mu.RLock()
	defer dl.mu.RUnlock()

	servers := make([]*protocol.ServerInfo, 0, len(dl.discovered))
...
func (dl *DiscoveryListener) addOrUpdateServer(info *protocol.ServerInfo) {
	dl.mu.Lock()
	defer dl.mu.Unlock()
