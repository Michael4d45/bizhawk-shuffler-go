...
func (psm *PluginSyncManager) fetchServerPlugins() (map[string]protocol.Plugin, error) {
	...
	var payload struct {
		Plugins map[string]protocol.Plugin `json:"plugins"`
	}
	if err := json.NewDecoder(resp.Body).Decode(&payload); err != nil {
		return nil, fmt.Errorf("decoding plugins JSON: %w", err)
	}

	enabled := make(map[string]protocol.Plugin)
...
func (psm *PluginSyncManager) scanLocalPlugins() (map[string]protocol.Plugin, error) {
	pluginDir := "./plugins"
	...
	out := make(map[string]protocol.Plugin)
	for _, e := range entries {
		...
		if f, err := os.Open(metaKV); err == nil {
			if err := protocol.ForEachKVLine(f, true, func(key, val string) error {
				switch key {
				case "name":
					p.Name = val
				case "version":
				...
				_ = f.Close()
				return nil, fmt.Errorf("read %s: %w", metaKV, err)
			}
			_ = f.Close()
		}

		settingsKV := filepath.Join(pluginDir, name, "settings.kv")
		p.Status = protocol.PluginStatusDisabled
		if f, err := os.Open(settingsKV); err == nil {
			if err := protocol.ForEachKVLine(f, false, func(key, val string) error {
				if strings.EqualFold(key, "status") {
					p.Status = protocol.PluginStatus(val)
				}
				return nil
			}); err != nil {
				_ = f.Close()
				return nil, fmt.Errorf("read %s: %w", settingsKV, err)
			}
			_ = f.Close()
		}
...
func (psm *PluginSyncManager) analyzeSyncRequirements(serverPlugins, localPlugins map[string]protocol.Plugin) (toDownload, toRemove []string) {
