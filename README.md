...
Data directory defaults to `%USERPROFILE%\BizShuffle\` (or `~/BizShuffle/`). **Host** starts the embedded server and opens the admin UI. **Join** installs BizHawk (and VC++ on Windows, or system Mono on Linux) via the dependencies panel when needed, then launches the emulator and connects to the server URL.
