...
local function get_current_canonical_game()
  local disp = get_rom_display_name()
...
local function do_swap(target_game)
  ...
  if cur and cur ~= "" and cur:lower() ~= "null" then
    ...
    pcall(function() save_state(path) end)
  end
...
local server = nil
do
  local ok, s = pcall(function() return socket.bind end)
  if ok and s then
    server = assert(socket.bind(HOST, PORT))
  else
    local c = socket.tcp()
    if c then
      local bind_ok, bind_err = pcall(function() return c:bind(HOST, PORT) end)
      if not bind_ok then
        local listen_ok, listen_err = pcall(function() return c:listen(PORT) end)
        if not listen_ok then
          error("socket bind/listen not available: " .. tostring(bind_err or listen_err))
        end
      else
        pcall(function() c:listen() end)
      end
      server = c
    else
...
local function send_line(line)
  if client_socket then
    local ok, err = pcall(function() client_socket:send(line .. "\n") end)
    if not ok then console.log("send error: " .. tostring(err)) end
  end
end
...
local function handle_line(line)
  ...
  if parts[1] == "CMD" then
    ...
    if cmd == "SWAP" then
      ...
      safe_exec_and_ack(id, function() schedule_or_now(at, function() do_swap(game) end, game) end)
    elseif cmd == "START" then
      local at, game = tonumber(parts[4]), parts[5]
      safe_exec_and_ack(id, function() schedule_or_now(at, function() do_start(game) end, game) end)
    elseif cmd == "SAVE" then
      local path = parts[4]
      safe_exec_and_ack(id, function() do_save(path) end)
    elseif cmd == "LOAD" then
      local path = parts[4]
      safe_exec_and_ack(id, function() do_load(path) end)
    elseif cmd == "PAUSE" then
      local at = tonumber(parts[4])
      safe_exec_and_ack(id, function() schedule_or_now(at, do_pause, 'pause') end)
    elseif cmd == "RESUME" then
      local at = tonumber(parts[4])
      safe_exec_and_ack(id, function() schedule_or_now(at, do_resume, 'resume') end)
    elseif cmd == "MSG" then
      local text = join_from(parts, 4)
      safe_exec_and_ack(id, function() show_message(text, 3) end)
    elseif cmd == "SYNC" then
      local game, state, state_at = parts[4], parts[5], tonumber(parts[6] or "0")
      safe_exec_and_ack(id, function()
        if state == "running" then
          if game and game ~= "" then
            schedule_or_now(state_at, function() do_start(game) end, game)
          end
        else
